...
function TimelineFormatter({ information }: { information: TimelineEntry[] }) {
...
function Timeline() {
  const [experienceData, setExperienceData] = useState<TimelineEntry[]>([]);
  const [educationData, setEducationData] = useState<TimelineEntry[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("https://2rf2ydlxu4qqup7eiovupi5moq0wfcnu.lambda-url.us-east-2.on.aws/")
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        const experiences = data.filter(
          (entry: TimelineEntry & { type: string }) => entry.type === "experience"
        );
        const education = data.filter(
          (entry: TimelineEntry & { type: string }) => entry.type === "education"
        );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loading ? <LoadingSpinner /> : <TimelineFormatter information={experienceData} />}
      </Tabs.Content>
      ...
      >
        <div className="absolute top-0 bottom-0 left-[35px] w-[2px] bg-gray-600"></div>
        {loading ? <LoadingSpinner /> : <TimelineFormatter information={educationData} />}
      </Tabs.Content>
    </Tabs.Root>
  );
}
